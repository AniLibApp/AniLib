...
open class DiscoverPopularFragment : DiscoverTrendingFragment() {
    ...
    private val order: Int
        get() = getDiscoverOrderFromType(DiscoverOrderType.POPULAR)

    private val isSectionEnabled: Boolean
        get() = isDiscoverOrderEnabled(DiscoverOrderType.POPULAR)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (isSectionEnabled) {
            ...
            if (savedInstanceState == null)
                viewModel.field = PopularMediaField.create().also {
    ...
    private fun handleClick(which: Int) {
        if (which == 0) {
            OpenSearchEvent(SearchFilterEventModel(sort = MediaSort.POPULARITY_DESC.ordinal)).postEvent
        } else if (which == 1) {
