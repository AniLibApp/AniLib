...
class DiscoverAiringViewModel(private val airingMediaService: AiringMediaService) :
    SourceViewModel<DiscoverAiringSource, AiringMediaField>() {

    val field = AiringMediaField()
    var adapter: Adapter? = null

    override fun createSource(field: AiringMediaField): DiscoverAiringSource {
        source = DiscoverAiringSource(field, airingMediaService, compositeDisposable)
        return source!!
    }
