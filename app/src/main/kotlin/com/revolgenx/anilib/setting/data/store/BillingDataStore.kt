...
class BillingDataStore(val dataStore: DataStore<Preferences>) {
    companion object{
        val appIsSupportedKey = booleanPreferencesKey("app_is_supported_key")
    ...
    val appIsSupported get() = dataStore.data.map { it[appIsSupportedKey] ?: false }

    suspend fun savePurchase(purchase: Purchase){
        dataStore.edit {
            it[appIsSupportedKey] = true
