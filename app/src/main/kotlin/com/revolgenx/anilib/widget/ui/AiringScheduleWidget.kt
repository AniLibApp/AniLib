...
class AiringScheduleWidget : GlanceAppWidget(), KoinComponent {
    private val airingScheduleService = get<AiringScheduleServiceImpl>()
            ...
            CompositionLocalProvider(LocalContext provides context) {
                GlanceTheme(colors = colors) {
                    Box(
                        ...
                        var resourceResult: ResourceState<PageModel<AiringScheduleModel>>? by remember {
                            mutableStateOf(ResourceState.loading())
                        }
                        ...
                        LaunchedEffect(refresh) {
                            ...
                            resourceResult = null
                            resourceResult = ResourceState.loading()
                            resourceResult = airingScheduleService.getAiringSchedule(field)
                                .map { ResourceState.success(it) }
                                .catch {
                                    Timber.d(it)
                                    emit(ResourceState.error(it))
                                }.single()
                            refresh = false
                        }
                        ...
                        Column {
                            Row(
                                ...
                                Column {
                                    GlanceText(
                                        text = startDateTime.dayOfWeek.getDisplayName(
                                            TimeTextStyle.FULL,
                                            Locale.getDefault()
                                        ),
                            ...
                            when (val resourceValue = resourceResult) {
