...
class ActivityUnionServiceImpl(apolloRepository: ApolloRepository, mediaSettingsPreferencesDataStore: MediaSettingsPreferencesDataStore) : BaseService(apolloRepository, mediaSettingsPreferencesDataStore),
                    ...
                    data = it.activities?.map {
                        ActivityUnionModel(
                            listActivityModel = it?.onListActivity
                                ?.takeIf {  if (field.canShowAdult) true else it.media?.isAdult == false }
                                ?.toModel(),
